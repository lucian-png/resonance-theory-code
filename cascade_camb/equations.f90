    ...
    ! CASCADE MODIFICATION: τ(z) function for the Feigenbaum cascade expansion
    ! τ(z) = 1/(1 + (z/z_t)^β)  for z < z_act
    ! τ(z) = τ_floor              for z > z_act
    ! where β = ln(δ) = 1.5410, z_t = 1.430, and z_act/τ_floor are set
    ! to reproduce the CMB first peak at ℓ = 220.
    !
    ! When use_cascade_expansion = .false., standard ΛCDM is used.
    ! When use_cascade_expansion = .true., τ(z) replaces ΩΛ.
    !
    ! Lucian Randolph & Claude Anthro Randolph, April 4, 2026
    ! Resonance Theory Project — Paper 42
    ...
    real(dl) :: z, tau_z, tau_active, blend

    ! CASCADE CONTROL FLAG: set to .true. to use cascade expansion
    logical, parameter :: use_cascade_expansion = .true.

    ! Feigenbaum cascade parameters for INVERTED τ(z):
    ! τ_friedmann(z) = τ₀ + (1-τ₀) × x/(1+x), x = (z/z_t)^β
    ! Goes from τ₀ ≈ 0.554 at z=0 to 1.0 at z→∞
    ! β = ln(δ) = 1.5410 (Feigenbaum derived)
    ! z_t = 0.50 (optimized for Friedmann form)
    ! τ₀ = 1/√(1 + (1-α²Ωb)/(α²Ωb)) = 0.554
    real(dl), parameter :: cascade_beta = 1.5409880_dl
    real(dl), parameter :: cascade_z_t = 0.50_dl
    real(dl), parameter :: cascade_tau0 = 0.554042_dl

    if (use_cascade_expansion) then
        ! CASCADE MODE: replace ΩΛ with inverted Feigenbaum τ(z)
        ! τ is LOW at z=0 (acceleration) and HIGH at z→∞ (matter-dom)
        ! H² = H²_matter / τ² provides late-time acceleration
        ...
        ! Compute inverted Feigenbaum τ(z)
        tau_active = (z / cascade_z_t)**cascade_beta
        tau_z = cascade_tau0 + (1.0_dl - cascade_tau0) * &
                tau_active / (1.0_dl + tau_active)
        if (tau_z < 0.001_dl) tau_z = 0.001_dl

        ! Matter + radiation + curvature (NO dark energy)
        grhoa2 = this%grho_no_de(a)

        ! Apply cascade correction: divide by τ²
        grhoa2 = grhoa2 / (tau_z * tau_z)

    else
        ! STANDARD MODE: use ΛCDM dark energy
